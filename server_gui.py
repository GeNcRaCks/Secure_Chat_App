...
class ServerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("🔐 Secure Chat – Bob (Server)")
    ...
    def setup_details_tab(self):
        ...
        flow_text = """1. ✅ RSA-2048 Key Pair Generated (Bob)
2. 🔄 Public Key Sent to Alice
3. ✅ Alice generates 256-bit AES Key
4. 🔄 AES Key encrypted with RSA & sent to Bob
5. ✅ Both parties now share AES Key
...
10. 🛡️ Integrity checked via HMAC-SHA256"""
    ...
    def start_server(self):
        try:
            ...
            self.root.after(0, self.update_status, "🟡 Listening on port 9999...", "yellow")
            ...
            while self.running:
                try:
                    ...
                    self.connected = True
                    self.root.after(0, self.update_status, f"🟢 Connected to {addr[0]}", "green")
                    ...
                    enc_aes = self.conn.recv(1024)
                    self.aes_key = decrypt_aes_key(enc_aes, self.private_key)
                    
                    self.root.after(0, self.update_connection_info)
                    
                    # Set timeout for receiving messages (30 seconds)
                            ...
                            if encrypted_msg:
                                ...
                                try:
                                    ...
                                    if integrity_ok:
                                        ...
                                        # Log to console
                                        print(f"[Bob] ✅ Message received (Integrity OK): {msg[:30]}...")
                                    else:
                                ...
                                self.connected = False
                                self.root.after(0, self.update_status, "🟡 Client disconnected", "yellow")
                                if self.conn:
                                    self.conn.close()
                                    self.conn = None
                                break
            ...
            if self.running:
                self.root.after(0, self.update_status, f"🔴 Server error: {str(e)}", "red")
    ...
    def restart_server(self):
        ...
        self.root.after(0, self.update_status, "🟡 Restarting server...", "yellow")
    ...
    def update_connection_info(self):
        ...
        info = f"""=== BOB'S KEYS ===
RSA Private Key: {self.private_key.export_key().decode('utf-8')[:80]}...
RSA Public Key: {self.public_key.export_key().decode('utf-8')[:80]}...
AES Key: {self.aes_key.hex()[:32]}...
HMAC Key (SHA-256): {hmac_key.hex()[:32]}...
Key Fingerprint: {get_key_fingerprint(self.public_key)}

=== ALICE'S KEYS ===
AES Key (shared): {self.aes_key.hex()[:32]}...
Encryption: Active ✅
Integrity: HMAC-SHA256 Active 🛡️"""
        ...
        details = get_encryption_details(self.aes_key, self.public_key)
    ...
    def send_message(self):
        ...
        if msg and self.connected:
            try:
                ...
                print(f"[Bob] Sent: '{msg[:30]}...' Hash: {msg_hash}")
    ...
    def send_test_message(self):
        if self.connected:
            ...
            try:
                ...
                print(f"[Bob] Test message sent. Integrity: {status}")
    ...
    def send_tampered_message(self):
        ...
        if self.connected:
            ...
            try:
                ...
                self.conn.send(tampered)
                print("[Bob] Sent TAMPERED message to test integrity check")
                ...
                print(f"[Bob] Tamper test error: {e}")

    def show_public_key(self):
        if self.public_key:
            key_str = self.public_key.export_key().decode('utf-8')
            messagebox.showinfo("Bob's Public Key", key_str)
